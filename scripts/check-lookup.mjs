...
import {
  caevForEventType,
  computeLookupVerdict as computeLookupVerdictRaw,
  daysOut,
  getScopeVendors,
  leadTimeProvenance,
  setScopeVendors,
...
// Legacy timing cases explicitly model a user who checked every vendor absent.
const computeLookupVerdict = (input) =>
  computeLookupVerdictRaw({
    getConfirmation: () => checkedAbsent,
    isPresentAtVendor: () => false,
    ...input,
  });
...
ok(
  "event applicability: rules.json rows + documented no-coverage (VettaFi rights-issue)",
);

// ─── Scope persistence ──────────────────────────────────────────────────────
const scopeStore = makeFakeStorage();
assert.equal(
  getScopeVendors(scopeStore).length,
  6,
  "virgin scope defaults to sourced vendors",
);
...
assert.deepEqual(
  getScopeVendors(scopeStore),
  ["solactive", "msci"],
  "unknown ids dropped, dupes deduped",
);
scopeStore.setItem("ca-hub.vendor-scope.v1", "{corrupt json");
assert.equal(
  getScopeVendors(scopeStore).length,
  6,
  "corrupt scope store falls back to the default",
);
ok(
  "vendor scope: default = sourced vendors, persisted subset, unknown ids dropped, corrupt store safe",
);
...
assert.equal(
  mixed.rows.find((r) => r.vendor === "vettafi")?.state,
  "not-applicable",
);
assert.equal(mixed.rows.find((r) => r.vendor === "ftse")?.state, "missing");
assert.equal(
  mixed.rows.find((r) => r.vendor === "msci")?.state,
  "not-assessed",
);
ok(
  "mixed scope: VettaFi (rights-issue) counted in notApplicable ONLY — never in a timing total",
);
...
assert.equal(
  unaGraded.empty,
  true,
  "nothing graded → honest empty, never '0 missing'",
);
assert.ok(verdictSummary(unaGraded.totals).includes("Confirmed at 0"));
assert.ok(
  !verdictSummary(unaGraded.totals, unaGraded.empty).includes("0 missing"),
);
ok(
  "not-assessed: excluded from covered/missing, reported separately, empty verdict",
);
...
assert.ok(
  !naSummary.includes("missing"),
  "an all-not-applicable scope never renders a timing gap",
);
...
assert.ok(
  verdictSummary(none.totals).includes("0 of 0 vendors state a position"),
);
...
ok(
  "FTSE stated 5-day lead, ex-date 10 days out → not-yet-due (timing, not a gap)",
);
...
assert.equal(
  userSet.rows[0]?.state,
  "not-yet-due",
  "1-day lead, ex-date 2 days out = not-yet-due",
);
assert.deepEqual(
  leadTimeProvenance(userSet.rows[0]?.source ?? "unset", "ftse"),
  {
    label: "your setting",
    tone: "user",
  },
...
ok(
  "user-set lead time: verdict re-graded AND provenance reads 'your setting' (D3)",
);
...
assert.equal(
  boundary.rows[0]?.state,
  "missing",
  "daysOut === leadDays is missing, not not-yet-due",
);
...
ok(
  "per-event override (9d) wins over stated 5d — still provenance 'your setting'",
);
...
ok(
  "unset lead time: provenance reads 'not set' — no number, no source, no verdict",
);
...
ok(
  "feed-present injection grades 'covered' — the counting already supports it",
);
...
assert.ok(summary.includes("Confirmed at 0, missing at 1"));
