...
assert.equal(snapshot.schema_version, "1.0"); assert.match(snapshot.snapshot_id, /^franklin-etf-\d{4}-\d{2}-\d{2}$/); assert.equal(snapshot.records.length, 127); assert.equal(snapshot.acquisition.record_count, 127);
...
assert.equal(snapshot.records.some((r) => r.index_provider === "Solactive AG"), true); assert.equal(snapshot.records.some((r) => r.ticker === "FLJP"), true); assert.deepEqual(new Set(["FLCB","FTNJ","FTNY","EZBC","FLGV","PBDC","YCLO","FLUD","MULT","FTMN","XRPZ","FLMI","FTPA","EZET","FTOH"]), new Set(snapshot.records.slice(-15).map((r) => r.ticker))); assert.equal(snapshot.records.filter((r) => r.index_type === "fixed-income").length, 19); assert.equal(snapshot.records.find((r) => r.ticker === "EZPZ").index_type, "thematic-custom");
