...
assert.equal(snapshot.schema_version, "1.0"); assert.match(snapshot.snapshot_id, /^franklin-etf-\d{4}-\d{2}-\d{2}$/); assert.equal(snapshot.records.length, 11); assert.equal(snapshot.acquisition.record_count, 11);
...
assert.equal(new Set(snapshot.records.map((r) => r.index_type)).size, 3, "reviewed table must exercise three index types");
