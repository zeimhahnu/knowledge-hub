...
assert.equal(snapshot.schema_version, "1.0"); assert.match(snapshot.snapshot_id, /^franklin-etf-\d{4}-\d{2}-\d{2}$/); assert.equal(snapshot.records.length, 112); assert.equal(snapshot.acquisition.record_count, 112);
...
assert.equal(snapshot.records.some((r) => r.index_provider === "Solactive AG"), true); assert.equal(snapshot.records.some((r) => r.ticker === "FLJP"), true); assert.deepEqual(new Set(["FFOG","FSML","IQM","FRIZ","INCM","PGRI","FTSD","FLIA","FTMH","FTMS","FTCA","EZPZ","FLBL","FLCO","TEMD"]), new Set(snapshot.records.slice(-15).map((r) => r.ticker))); assert.equal(snapshot.records.filter((r) => r.index_type === "fixed-income").length, 8); assert.equal(snapshot.records.find((r) => r.ticker === "EZPZ").index_type, "thematic-custom");
