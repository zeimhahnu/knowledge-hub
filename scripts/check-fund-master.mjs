...
assert.equal(snapshot.schema_version, "1.0"); assert.match(snapshot.snapshot_id, /^franklin-etf-\d{4}-\d{2}-\d{2}$/); assert.equal(snapshot.records.length, 177); assert.equal(snapshot.acquisition.record_count, 177);
...
assert.equal(snapshot.records.some((r) => r.index_provider === "Solactive AG"), true); assert.equal(snapshot.records.some((r) => r.ticker === "FLJP"), true); const requiredTail = ["FLHY","FTMA","CNV","FGOV","FHIS","FLCI","FLCP","FLGA","FLSD","IE00BHZRR253","IE000H0TSO96","IE00BFWXDX52","IE000E02WFD5","BNDS","FRAR"]; for (const ticker of requiredTail) assert(tickers.has(ticker), `required prior record missing: ${ticker}`); const requiredNew = ["FSPW","FTCD","FTCS","FTFN","FTTC","FVCH","FVEM","FVSJ","UDVD","USMC"]; for (const ticker of requiredNew) assert(tickers.has(ticker), `required new record missing: ${ticker}`); assert.equal(snapshot.records.filter((r) => r.index_type === "fixed-income").length, 37); assert.equal(snapshot.records.find((r) => r.ticker === "EZPZ").index_type, "thematic-custom");
assert.equal(new Set(snapshot.records.map((r) => r.index_type).filter((type) => type !== null)).size, 6, "reviewed table must exercise six index types");
