...
export function CaAnalystPanel({ context }: { context: AnalystLookupContext }) {
  ...
  return (
    <SurfaceSection className="space-y-4" aria-labelledby="ca-analyst-heading">
      ...
      {busy && <p role="status" className="flex items-center gap-2 text-sm text-muted-foreground"><LoaderCircleIcon className="h-4 w-4 motion-safe:animate-spin" aria-hidden /> Analyst {turn.stage ?? "working"}…</p>}
