...
export type FundResolution = { mode: "fund-resolved"; ticker: string; fund: FundMasterRecord; indexType: IndexType; ruleScope: "3-d"; warnings: string[] } | { mode: "fund-unresolved"; ticker: string; reason: "unknown-ticker" | "missing-index-fields"; ruleScope: "2-d"; warnings: string[] } | { mode: "p0-compat"; ruleScope: "2-d"; warnings: [] };
...
export function resolveFundRules(selectedFundTicker: string | undefined, snapshot: FranklinEtfSnapshot, rules: VendorRule[]): { resolution: FundResolution; rows: VendorRule[] } {
  ...
  if (!fund) return { resolution: { mode: "fund-unresolved", ticker, reason: "unknown-ticker", ruleScope: "2-d", warnings: [`${ticker} is not in the reviewed Franklin snapshot; using the existing P0 2-D rules.`] }, rows: rules };
